...
```text
loop_state.py — модель сознания EVA на основе ленты Мёбиуса  
handlers.py — логика диалога, traversal и композиция  
llm_interface.py — обёртка для LLM, настройка тона и контекста  
app.py — Flask-приложение, маршруты и запуск  
```
...
```json
User: "Мне грустно"
EVA: "Я рядом. Хочешь, я помогу тебе прожить это чувство?"
```



```text
© Diana, 2025. Все права на архитектуру EVA.Möbius принадлежат автору.  
Распространение и модификация только с указанием авторства.
```
